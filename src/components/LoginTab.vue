<template>
  <n-config-provider :themeOverrides="themeOverrides">
    <form
      class="login-tab-main"
      @submit.prevent
    >
      <p class="title">Picnia</p>
      <Transition>
        <n-input
          @change="debounce(check, 200)('username', userName)"
        ...
        >
          <template #suffix>
            <n-icon
              :component="Checkmark"
              v-if="userNameError === false && !userNameLoading"
            ></n-icon>
          </template>
        </n-input>
      </Transition>
      <n-input
        @change="debounce(check, 200)('useremail', email)"
      ...
      >
        <template #suffix>
          <n-icon
            :component="Checkmark"
            v-if="email.length && userEmailError === false && !userEmailLoading"
          ></n-icon>
        </template>
      </n-input>
      <n-input
      ...
      <p
        ...
        "
      >
        {{ isRegister ? '已有账号？点击登陆！' : '还没有加入Picnia吗？点击注册！' }}
      </p>
    </form>
  </n-config-provider>
</template>
